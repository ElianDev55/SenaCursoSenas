...
const usePutDiscussion = () => {
    ...
    try {
        await axios.put(`http://127.0.0.1:8000/discussion/${videoId}`, updatedData);
        setUpdated(false);
    } catch (error) {
        setUpdated(false);
...
const useDeleteDiscussion = () => {
    ...
    console.log(videoId);
    try {
        await axios.delete(`http://127.0.0.1:8000/discussion/${videoId}`);
        setDeleted(false);
    } catch (error) {
        setDeleted(false);
...
function useSearchDiscussion(initialSearchTerm = '') {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get('http://127.0.0.1:8000/discussion/');
...
export {useFetchDiscussion,useSendDataDiscussion,useDeleteDiscussion,usePutDiscussion,useSearchDiscussion};
