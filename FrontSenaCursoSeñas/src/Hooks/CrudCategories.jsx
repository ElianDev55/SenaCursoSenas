...
const usePutCategories = () => {
    ...
    try {
        await axios.put(`http://127.0.0.1:8000/categories/${videoId}`, updatedData);
        setUpdated(false);
    } catch (error) {
        setUpdated(false);
...
const useDeleteCategories = () => {
    ...
    console.log(videoId);
    try {
        await axios.delete(`http://127.0.0.1:8000/categories/${videoId}`);
        setDeleted(false);
    } catch (error) {
        setDeleted(false);
...
function useSearchCategories(initialSearchTerm = '') {
...
export {useFetchCategories,useSendDataCategories,useDeleteCategories,usePutCategories,useSearchCategories};
